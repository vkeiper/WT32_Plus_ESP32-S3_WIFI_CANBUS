...
void CfgSetArrayData(unsigned char *setdata, unsigned int SlotName, unsigned char len)
{
	...
	for(i=0;i<s_cfgdata.index ;i++)
	{
			...
			{	
					...
					// index the data for the SLOT we passed in is located at
					//startaddress = CNTRLCFGOFFSET + s_cfgdata.offset[i];
		
//					for(block=0;block<NORQTYBLOCKS;block++)
//					{
//							if(s_cfgdata.block[i]==block)
//							{
//								stadr_blk = (s_cfgdata.block[i]-1) * 0xFFFF; // start address for each block data = block has 0xFFFF of 16 bit data blocks 128KB per block
//								break;
//							}
//					}
					// realistically lok this to block 1 128KB is plenty of data for configuration
					//if( s_cfgdata.block[i] ==1)
					//{
					//		stadr_blk = 0;
					//}
					//else 
					//if( s_cfgdata.block[i] ==2)
					//{
				///			stadr_blk = 0x00020000;// start address of block 2 in NOR
				//	}
					memset(lclbuff,0x00,256);
					if(BSP_NOR_ReadData(stadr_blk, (uint16_t*)lclbuff, 128) != NOR_STATUS_OK)
					{
							while(1);
					}
					...
					{
							lclbuff[j] = setdata[l]; //copy passed in data to local buffer					
							l++;
					}
					...
					{
							if(BSP_NOR_WriteData(stadr_blk, (uint16_t*)lclbuff, 128) != NOR_STATUS_OK)
							{
									GuiState.statusreg &= 0xff7f; //and off bit 7 NOR FLASH bit
									while(1);
							}
...
//0
	...
	s_cfgdata.block[i]       = (s_cfgdata.offset[i]/NORBLKSZ)+1; // determine what block of NOR in which this data resides
//1
	i++;
...
//7
  ...
  s_cfgdata.slotname[i]    = PRFL0;
	s_cfgdata.offset[i]      = s_cfgdata.offset[i-1] + s_cfgdata.numbytes[i-1];  //offset
	s_cfgdata.numbytes[i]    = 32; //length
