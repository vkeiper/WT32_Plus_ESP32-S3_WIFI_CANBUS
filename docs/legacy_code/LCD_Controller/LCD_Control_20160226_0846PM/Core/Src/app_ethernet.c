...
void User_notification(struct netif *netif) 
{
  if (netif_is_up(netif))
 {
    ...
    //LCD_UsrLog ("Static IP address: %s\n", iptxt);
		sprintf((dbgstrip)"Stat IP %s\n", iptxt);
//	  GUI_SetColor(GUI_WHITE);
//		GUI_SetFont(GUI_FONT_13B_ASCII);
//		GUI_DispStringAt( "Looking for DHCP sever", 50, 4);
#else    
...
//	  GUI_SetColor(GUI_WHITE);
//		GUI_SetFont(GUI_FONT_13B_ASCII);
//		GUI_DispStringAt( "No Cable", 50, 4);
#else    
...
void ethernetif_notify_conn_changed(struct netif *netif)
{
  ...
  if(netif_is_link_up(netif))
  {
    ...
    //LCD_UsrLog ("The network cable is now connected \n");
    //LCD_UsrLog ("Static IP address: %s\n", iptxt);
		sprintf(dbgstrip," Connected Stat IP %s\n", iptxt);
//		GUI_SetColor(GUI_WHITE);
//		GUI_SetFont(GUI_FONT_13B_ASCII);
//		GUI_DispStringAt( "Looking for DHCP sever", 50, 4);
#else
    ...
    case DHCP_START:
      {
       ...
       //LCD_UsrLog ("  State: Looking for DHCP sever ...\n");
			 //GUI_SetColor(GUI_WHITE);
			 //GUI_SetFont(GUI_FONT_13B_ASCII);
			 //GUI_DispStringAt( "Looking for DHCP sever", 140, 0);
#endif
      }
      break;
    ...
    case DHCP_WAIT_ADDRESS:
      {        
        ...
        //LCD_UsrLog ("IP address assigned by a DHCP server: %s\n", iptxt);
				sprintf(dbgstrip,"IP:%s\n", iptxt);
//				GUI_SetColor(GUI_WHITE);
//				GUI_SetFont(GUI_FONT_13B_ASCII);
//				GUI_DispStringAt( (char*)dbgstrip, 140, 0);
#else
     BSP_LED_On(LED1);   
#endif 
        }
        else
        {
          ...
          ///LCD_UsrLog ("Static IP address  : %s\n", iptxt);
//					sprintf(dbgstrip,"DHCP ERR: %s\n", iptxt);
//					GUI_DispStringAt( (char*)dbgstrip, 50, 4);
#else
     BSP_LED_On(LED1);  
#endif
          }
