...
#define ID_BTN_VRD (GUI_ID_USER + 0x10)    
#define ID_BTN_IRD (GUI_ID_USER + 0x11)  
#define ID_BTN_REF0 (GUI_ID_USER + 0x12)  
#define ID_BTN_REF1 (GUI_ID_USER + 0x13)  
#define ID_BTN_REF2 (GUI_ID_USER + 0x14)  
#define ID_BTN_REF3 (GUI_ID_USER + 0x15)  
#define ID_BTN_HM (GUI_ID_USER + 0x16)  
#define ID_TXT_DATLBL (GUI_ID_USER + 0x17)
...
**********************************************************************
*/
static GRAPH_DATA_Handle  _ahData[6]; // Array of handles for the GRAPH_DATA objects
static GRAPH_SCALE_Handle _hScaleV;   // Handle of vertical scale
static GRAPH_SCALE_Handle _hScaleH;   // Handle of horizontal scale

static I16 _aValue[6];
static int _Stop = 0;
static uint8_t b_btntog[8];// only using 3 btns currently 

static GUI_COLOR _aColor[] = {GUI_RED, GUI_GREEN, GUI_LIGHTBLUE,GUI_ORANGE,GUI_WHITE,GUI_LIGHTCYAN }; // Array of colors for the GRAPH_DATA objects

//
// Dialog ressource
//
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  { FRAMEWIN_CreateIndirect, "Data Log Graph Screen",  0                ,   0,   30, 480, 240, FRAMEWIN_CF_MOVEABLE },
  ...
  { BUTTON_CreateIndirect,    "Full Screen",       GUI_ID_BUTTON0   , 480-85, 240-65,  65,  30 },
  { CHECKBOX_CreateIndirect,  0,                   GUI_ID_CHECK8    , 235, 205,  70,   0 },
	{ TEXT_CreateIndirect,      "Choose Data",ID_TXT_DATLBL,480-(85*2),   0, 140,  20,  20 },
	{ BUTTON_CreateIndirect,    "Vrdb",       ID_BTN_VRD    , 480-85*2, 35*0+25,  65,  30 },
	{ BUTTON_CreateIndirect,    "Irdb",       ID_BTN_IRD    , 480-85*2, 35*1+25,  65,  30 },  
	{ BUTTON_CreateIndirect,    "Ref0",       ID_BTN_REF0   , 480-85*2, 35*2+25,  65,  30 },
	{ BUTTON_CreateIndirect,    "Ref1",       ID_BTN_REF1   , 480-85*1, 35*0+25,  65,  30 },
	{ BUTTON_CreateIndirect,    "Ref2",       ID_BTN_REF2   , 480-85*1, 35*1+25,  65,  30 },
  { BUTTON_CreateIndirect,    "Ref3",       ID_BTN_REF3   , 480-85*1, 35*2+25,  65,  30 },
  { BUTTON_CreateIndirect,    "Home",       ID_BTN_HM     ,   480-85, 240-(65 +35*1),  65,  30 },
};
...
static void _AddValues(void)
{
  ...
  for (i = 0; i < GUI_COUNTOF(_aColor); i++) 
	{
			int Add = ((unsigned)rand()) % (2 + (i>>2) * (i>>2));
			int Vz  = (((unsigned)(rand()) % 2) << 1) - 1;
			//int Vz  = (unsigned)(rand() % 2);
			...
			// Only add value if dat oblect is attached to graph
			if(_aUserdata[i][0] & 0x01)
			{
...
*
* Function description
* This routine hides/shows all widgets except the back\fullscreen button, graph and scroll bar widgets
* Obviously I used to make the graph larger
*
*/
...
static void _cbCallback(WM_MESSAGE * pMsg) {
  ...
  switch (pMsg->MsgId) 
	{
					// I have a timer callback in the k_menu src file that invalidates the window and causes a repaint
					// at 100mS interval
			case WM_PAINT:  
					 UpdateGraphData();
		  ...
		  break;
			...
			case WM_INIT_DIALOG:
						...
						GuiState.gph_scn_state = G_GPH_INIT;
						
						//
						//
						// Initialization of 'Load Select Label_txt'
						//
						hItem = WM_GetDialogItem(pMsg->hWin, ID_TXT_DATLBL);
			...
			case 	WM_NOTIFICATION_RELEASED:
						...
						{			
								...
								case ID_BTN_REF3:									
											...
											switch (Id) 
											{
														case ID_BTN_VRD: // Notifications sent by 'Checkbox Reference Electrode 0'
																	dataidx =0;
																	chkidx = ID_BTN_VRD;
														break;

														case ID_BTN_IRD: // Notifications sent by 'Checkbox Reference Electrode 1'
																	dataidx =1;
																	chkidx = ID_BTN_IRD;
														break;

														case ID_BTN_REF0: // Notifications sent by 'Checkbox Reference Electrode 2'
																	dataidx =2;
																	chkidx = ID_BTN_REF0;
														break;
														
														case ID_BTN_REF1: // Notifications sent by 'Checkbox Reference Electrode 2'
																	dataidx =3;
																	chkidx = ID_BTN_REF1;
														break;
														
														case ID_BTN_REF2: // Notifications sent by 'Checkbox Reference Electrode 2'
																	dataidx =4;
																	chkidx = ID_BTN_REF2;
														break;
														
														case ID_BTN_REF3: // Notifications sent by 'Checkbox Reference Electrode 2'
																	dataidx =5;
																	chkidx = ID_BTN_REF3;
														break;
											}
											
											hItem = WM_GetDialogItem(pMsg->hWin, chkidx);
											
											if(!b_btntog[dataidx])
											{
															BUTTON_SetPressed(hItem,1);
															hItem = WM_GetDialogItem(pMsg->hWin, GUI_ID_GRAPH0);
															_aValue[dataidx] = rand() % 180;
															if(!_aUserdata[dataidx][0] & 0x01)
															{	
																	_ahData[dataidx] = GRAPH_DATA_YT_Create(_aColor[dataidx], 500, 0, 0);
																	GRAPH_AttachData(hItem, _ahData[dataidx]);
																	_aUserdata[dataidx][0] |= 0x01;// OR on bit0
																	GRAPH_SetUserData(hItem,_aUserdata,1);
															}
											}
											else
											{
															...
															if(_aUserdata[dataidx][0] & 0x01)
															{	
																	_aUserdata[dataidx][0] &= 0xFE; //clear bit0 in user data to tell adddata routine not to adddata
																	GRAPH_DetachData(hItem, _ahData[dataidx]);
																	GRAPH_DATA_YT_Delete(_ahData[dataidx]);
															}
											}
											_Stop = 0;
											b_btntog[dataidx]  = !b_btntog[dataidx];
											
								break;
											
								/**************/
											
							 case ID_BTN_HM: // Notifications sent by 'Home_Btn'
										switch(NCode) 
										{
													case WM_NOTIFICATION_RELEASED:
...
void UpdateGraphData()
{
	...
	{
      if (!hGraph) {
        hGraph = WM_GetDialogItem(hDlg_gbl, GUI_ID_GRAPH0);
      }
      _AddValues();
  }
...
WM_HWIN GetWinHandleiccp_gph(void);
WM_HWIN GetWinHandleiccp_gph(void)
{
	return hDlg_gbl;
}
