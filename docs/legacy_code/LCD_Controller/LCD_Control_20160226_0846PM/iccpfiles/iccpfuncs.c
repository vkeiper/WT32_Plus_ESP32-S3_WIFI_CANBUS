			...
			case I_STARSP:
					...
					if(GuiState.SysDataPSU[psuidx].Status & 0x0008)
					{
						  ...
						  i_starsp_timeout_cnt=0;
							...
							// write curent status to datalog for this psuidx
							// Currently not working, causes LCD reset
							// 10-21-15 needs to be debugged
//							WriteToLog((char *)uSDpath,
//										&GuiState.sec,&GuiState.min,&GuiState.hour,
//										&GuiState.month,&GuiState.day,&GuiState.year, 
//										&GuiState.SysDataPSU[psuidx].Vrdbk,
//										&GuiState.SysDataPSU[psuidx].Irdbk,
//										&GuiState.SysDataPSU[psuidx].PsuTemp,
//										&psuidx);
					}
...
//	tmr_cntr[3]++;
	...
	{
			//while(b_CANTXFlag);
		//(GuiState.SysDataPSU[i].Status & 0x0008)
			//if(!b_CANTXFlag)// if the last time in the state machine a CANTX
		  if((!b_CANTXFlag)||!(BITCHK(GuiState.SysDataPSU[i].Status,6)))// if the last time in the state machine a CANTX or transmitting profile
...
void CopyProfileStructRam2Ram(IccpPrfParam_t *pflsrc,IccpPrfParam_t *pfldst)
{
		...
		//pfldst->refeleclongname = pflsrc->refeleclongname; //EAJ 2016/01/12
